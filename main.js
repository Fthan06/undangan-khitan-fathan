...
function initNavDots() {
  ...
  ['hero', 'profile', 'countdown', 'payment', 'location', 'wishes', 'ending']
...
function initPhotoObserver() {
  ...
  const dots = Array.from(dotsWrap.querySelectorAll('.sl-dot'));
  ...
  function goTo(next) {
    ...
    setTimeout(() => slides[cur]?.classList.remove('prev'), 950);
  ...
  function startAuto() {
    clearInterval(timer);
    timer = setInterval(() => goTo(cur + 1), 2500);
  }
  ...
  function startSlider() {
    ...
    slides[cur].querySelector('img')?.classList.add('colored');
    startAuto();
  }
